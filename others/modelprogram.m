...
val = h ;
h
matrixret = chooseoption1(I_rgb,val,matrixret);
...
for i=1:count
    for j=1:count
        if i~=j
           ed = 1/(sqrt((Groups1(i,2)-Groups1(j,2))^2 + (Groups1(i,3)-Groups1(j,3))^2));
        if ed <0.000001
            counted=counted+1;
        end
...
clearvars -except h kval max_label labels finalimage imagecopy matrixret I_gray I_rgb rows_lab columns_lab AChannel BChannel a1 a2 a3 asal1 asal2 asal3 hl hu sl su Groups3;
end

finalimage1 = sqrt(a1.^2 + a2.^2 + a3.^2);
salimage = sqrt(asal1.^2 + asal2.^2 + asal3.^2);
figure(16);imshow(salimage);
imwrite(salimage,'salimage.jpg');
figure(8); imshow(finalimage);
imwrite(finalimage1,'finalim.jpg');
figure(9);imshow(finalimage1);
...
figure(15);imshow(finalimage2);


finalimagenorm = finalimage2;

figure(20);imshow(finalimagenorm);
max_label
