...
saldir='./our output slic images/';% the output path of the saliency map
mkdir(saldir);
...
% MinimumRegionArea=700;
I=imread(imname);
imshow(imname);
[l, Am, C] = slic(I, 100, 25, 1, 'median');
lc = spdbscan(l, C, Am, 5);
fimg1 = drawregionboundaries(lc, I, [255 255 255]);
...
for i=1:rows_lab
    for j=1:columns_lab
        ...
        if Groups(labels(i,j),4) < 0
            fprintf('%d %d %f %f',i,j,Groups(labels(i,j),4),matrixret(i,j));
            l=input('\nfgfg=');
        end
    end
...
for i=1:count
    for j=1:count
        ...
        if i~=j
         ...
         ed = exp((-1*((Groups(i,2)-Groups(j,2))^2 + (Groups(i,3)-Groups(j,3))^2))/(2*sigma^2)) + 1;
        
         productdist=(Df(i,j)*ed);
         F(i,j) = productdist;
        else
         F(i,j) = 0;
        end
...
for j=1:rows_lab
    for k=1:columns_lab
        ...
        if yesno == 1
%             if degreenet(pos) > (cutoff*max(degreenet))
...
maxB = max(a1(:));
minB = min(a1(:));
a1 = (a1 - minB) / (maxB - minB);

maxB = max(a2(:));
minB = min(a2(:));
a2 = (a2 - minB) / (maxB - minB);

maxB = max(a3(:));
minB = min(a3(:));
a3 = (a3 - minB) / (maxB - minB);

maxB = max(asal1(:));
minB = min(asal1(:));
asal1 = (asal1 - minB) / (maxB - minB);

maxB = max(asal2(:));
minB = min(asal2(:));
asal2 = (asal2 - minB) / (maxB - minB);

maxB = max(asal3(:));
minB = min(asal3(:));
asal3 = (asal3 - minB) / (maxB - minB);

% finalimage1 = sqrt(a1.^2 + a2.^2 + a3.^2);
% salimage = sqrt(asal1.^2 + asal2.^2 + asal3.^2);

finalimage1 = (a1 + a2 + a3)/3;
salimage = (asal1 + asal2 + asal3)/3;
